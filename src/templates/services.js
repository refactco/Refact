    ...
    const handleScroll = () => {
      ...
      for (let i = sectionElements.length - 1; i >= 0; i--) {
        if (
          ...
          setActiveItemId(
            serviceSection.services[i].title.replace(/\s+/g, '-').toLowerCase()
          );
          break;
        }
      }
          ...
          <div className="c-services__wrap">
                    ...
                    <ul className="c-services__sub-list">
                      {service.subList.map((item, index) => (
                        <li className="c-services__sub-list-item" key={index}>
                          ...
                          {item.description && (
                            <div
                              className="c-services-sub-list__desc"
                          ...
                          {item.activateSubitem && (
                            <ul className="c-services__subitems-list">
                              {item.subItem.map((subItem, index) => (
                                <li
                                  className="c-services-subitems__item"
                                  key={index}
                                  dangerouslySetInnerHTML={{
            ...
            <div className="c-services__sidebar">
              ...
              <div className="c-services-sidebar__list">
                {serviceSection.services.map((service, index) => (
                  <div
                    className={`c-services-sidebar__item ${
                      activeItemId ===
                      service.title.replace(/\s+/g, '-').toLowerCase()
                        ? 'is-active'
                        : ''
                  ...
                  >
                    <a
                      href={`#${service.title
                        .replace(/\s+/g, '-')
                        .toLowerCase()}`}
                      className="c-services-sidebar__link"
                      onClick={handleClick}
                    >
                      <div className="c-service-sidebar__icon">
                        ...
                        >
                          <path
                            fill="#59CC51"
                            d="M9 9.796c.44 0 .796-.356.796-.796V1.838a.796.796 0 1 0-1.591 0v6.366H1.838a.796.796 0 1 0 0 1.592H9ZM.437 1.562l8 8 1.126-1.125-8-8L.437 1.562Z"
                          />
                        </svg>
                      </div>
                      {service.title}
