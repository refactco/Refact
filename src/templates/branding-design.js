  ...
  const renderSection = (section, index) => {
    switch (section.fieldGroupName) {
      ...
      case 'Template_PageBuilder_Pagebuilder_PageBuilder_ToolsResources':
        return (
          <ContainerBox key={index} className={`c-section--goodtech is-branding-page has-section-${index}`}>
...
export function Head({ data }) {
  ...
  return (
    <>
      ...
      // featuredImage={post.seo.opengraphImage.localFile.url} 
      />
    </>
  );
}
...
export const pageQuery = graphql`
  query {
    wpPage(slug: {eq: "branding-and-design"}) {
      id
      seo {
        title
        metaDesc
